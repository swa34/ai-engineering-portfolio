...
export async function checkAccessibility(page: Page, checkpoint: string) {
  ...
  expect(result.violations, `Accessibility violations at ${checkpoint}`).toEqual([]);
}
...
export async function checkNarrowReflow(page: Page) {
  ...
  expect(dimensions.content, 'Page must not scroll horizontally at 320 CSS pixels').toBeLessThanOrEqual(dimensions.viewport);
  expect(dimensions.body, 'Body must reflow at 320 CSS pixels').toBeLessThanOrEqual(dimensions.viewport);
}
...
export function monitorExternalRequests(page: Page) {
  ...
  page.on('request', (request) => {
    ...
    if (['http:', 'https:'].includes(url.protocol) && url.origin !== 'http://localhost:3001') {
