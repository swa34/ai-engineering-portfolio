...
async function administer(path: string, data: unknown, method = "POST") {
	...
	try {
		const selected = await context.post("/api/v1/demo-session", {
...
async function keyboardActivate(page: Page, control: Locator, key = "Enter") {
	...
	throw new Error(
		`Control was not keyboard reachable: ${await control.textContent()}`,
	);
}

async function createExample(page: Page, keyboard = false) {
	const activate = (control: Locator, key = "Enter") =>
		keyboard ? keyboardActivate(page, control, key) : control.click();
	await page.goto("/");
	const contributor = page.getByRole("button", {
		name: /contributor.*Create fictional/,
	});
	...
	await activate(
		page.getByRole("button", { name: "New announcement", exact: true }),
	);
	...
	const consent = page.getByRole("checkbox", {
		name: /I give fictional demo permission/,
	});
...
async function submitExample(page: Page, keyboard = false) {
	...
	if (keyboard) await keyboardActivate(page, submit);
	else await submit.click();
	await expect(
		page.getByRole("heading", { name: "Your source is ready." }),
	).toBeVisible();
	const detail = await (
		await page.request.get(`/api/v1/cycles/${cycleId}`)
	).json();
...
async function switchRole(
	...
	if (keyboard) await keyboardActivate(page, trigger);
	else await trigger.click();
	const dialog = page.getByRole("dialog", { name: "Switch simulated role" });
	const selection = dialog.getByRole("button", {
		name: new RegExp(`^${next}`),
	});
	if (keyboard) await keyboardActivate(page, selection);
...
test("complete-standard: keyboard approval, exact artifact, reflow, print, and isolated mock requests", async ({
	page,
}) => {
	...
	await page.goto("/");
	await expect(
		page.getByRole("button", { name: /contributor.*Create fictional/ }),
	).toBeVisible();
	...
	await expect(
		page.getByText("Unapproved fictional draft", { exact: true }),
	).toBeVisible();
	await page.emulateMedia({ media: "screen" });
	await keyboardActivate(
		page,
		page.getByRole("button", { name: "Generate candidate", exact: true }),
	);
	await expect(
		page.getByRole("heading", { name: "Source checks passed" }),
	).toBeVisible();
	await expect(
	...
	await expect(
		page.getByRole("navigation", { name: "Announcement cycles" }),
	).toContainText("Validation: passed");
	await page.screenshot({
		path: test.info().outputPath("review-desktop.png"),
		fullPage: true,
	});
	await checkNarrowReflow(page);
	await checkAccessibility(page, "review-320px");
	await page.screenshot({
		path: test.info().outputPath("review-320px.png"),
		fullPage: true,
	});
	...
	const approvalDialog = page.getByRole("dialog", {
		name: "Approve this exact revision?",
	});
	await expect(
		approvalDialog.getByRole("button", { name: "Cancel" }),
	).toBeFocused();
	...
	await keyboardActivate(
		page,
		approvalDialog.getByRole("button", {
			name: "Confirm approval of revision 1",
		}),
	);
	await expect(
		page.getByRole("heading", { name: "Approved. Ready to prepare." }),
	).toBeVisible();
	await keyboardActivate(
		page,
		page.getByRole("button", {
			name: "Prepare approved artifact",
			exact: true,
		}),
	);
	const downloadLink = page.getByRole("link", {
		name: "Download approved text",
		exact: true,
	});
	...
	for (const value of [
		...
		detail.current_revision.content.announcement_body,
		detail.current_revision.content.short_social_version,
	])
		expect(text).toContain(value);
	const exportPath = (await downloadLink.getAttribute("href"))!.replace(
		"/download",
		"",
	);
	...
	await expect(
		page.getByRole("button", { name: "Print approved preview" }),
	).not.toBeVisible();
	expect(external).toEqual([]);
	await test.info().attach("artifact-identifiers", {
		body: JSON.stringify({
			cycleId,
...
test("invalid source and independent acknowledgments preserve input and expose linked errors", async ({
	page,
}) => {
	...
	await page
		...
		.click();
	await expect(error).toBeFocused();
	await expect(page.getByRole("combobox", { name: /Degree/ })).toHaveAttribute(
		"aria-invalid",
		"true",
	);
...
test("timeout remains retryable and only an explicit later generation creates a revision", async ({
	page,
}) => {
	...
	await page
		.getByRole("button", { name: "Generate candidate", exact: true })
		.click();
	await expect(page.getByRole("status")).toContainText("Generation failed", {
		timeout: 18_000,
	});
...
test("altered facts and a human invention block approval; explicit restoration preserves revisions", async ({
	page,
}) => {
	...
	await expect(
		page.getByRole("heading", { name: "Resolve blocking findings" }),
	).toBeVisible();
	await expect(
		page.getByRole("button", { name: "Approve revision…" }),
	).toBeDisabled();
	...
	await page
		...
		.fill("Avery Example receives an invented award");
	await page
		.getByRole("button", { name: "Save new revision", exact: true })
		.click();
	await expect(
		page.getByRole("heading", { name: "Current revision 2", exact: true }),
	).toBeVisible();
	await expect(
		page.getByRole("button", { name: "Approve revision…" }),
	).toBeDisabled();
	...
	await page
		.getByRole("button", { name: "Restore source-based wording…" })
		.click();
	const dialog = page.getByRole("dialog", { name: "Preview restored wording" });
	await expect(dialog.getByRole("button", { name: "Cancel" })).toBeFocused();
	await dialog
		.getByRole("button", { name: "Save restored wording as new revision" })
		.click();
	await expect(
		page.getByRole("heading", { name: "Current revision 3", exact: true }),
	).toBeVisible();
	await expect(
		page.getByRole("heading", { name: "Source checks passed" }),
	).toBeVisible();
	...
	await expect(
		page.getByRole("heading", { name: "Historical revision 1", exact: true }),
	).toBeVisible();
	await expect(revisions).toBeFocused();
});

test("stale editor preserves unsaved text and requires an explicit refresh", async ({
	page,
}) => {
	...
	const otherEdit = await page.request.post(
		`/api/v1/cycles/${cycleId}/revisions`,
		{
			data: {
...
test("a revision changed behind an open approval dialog cannot be approved", async ({
	page,
}) => {
	const cycleId = await generateExample(page);
	await switchRole(page, "contributor", "reviewer");
...
test("request changes requires a reason; rejection is terminal and restart copies only facts", async ({
	page,
}) => {
	await generateExample(page);
	await switchRole(page, "contributor", "reviewer");
	...
	await expect(
		changes.getByRole("button", { name: "Confirm request for changes" }),
	).toBeDisabled();
	await changes
		.getByRole("textbox", { name: /Reason/ })
		.fill("Please verify the fictional announcement wording.");
	await changes
		.getByRole("button", { name: "Confirm request for changes" })
		.click();
	await expect(
	...
	await reject
		.getByRole("textbox", { name: /Reason/ })
		.fill("A separate fictional source review is required.");
	...
	await expect(
		page.getByRole("checkbox", { name: /I confirm this record/ }),
	).not.toBeChecked();
	await expect(
		page.getByRole("checkbox", { name: /I give fictional demo permission/ }),
	).not.toBeChecked();
});

test("HTML-like source is rendered as inert text through mock generation", async ({
	page,
}) => {
	await createExample(page);
	...
	page.on("request", (request) => {
		if (new URL(request.url()).pathname === "/x") requests.push(request.url());
	});
	await page
		.getByRole("button", { name: "Submit source", exact: true })
		.click();
	await expect(
		page.getByRole("heading", { name: sentinel, exact: true }),
	).toBeVisible();
	await page
		.getByRole("button", { name: "Generate candidate", exact: true })
		.click();
	...
	expect(
		await page.evaluate(
			() => (window as Window & { __injected?: boolean }).__injected,
		),
	).toBeUndefined();
...
test("withdrawal blocks protected actions and correction requires fresh acknowledgments", async ({
	page,
}) => {
	await generateExample(page);
	...
	await expect(dialog.getByRole("button", { name: "Cancel" })).toBeFocused();
	await dialog
		.getByRole("button", { name: "Withdraw consent for this source" })
		.click();
	await expect(
		page.getByRole("strong").filter({ hasText: /^Consent withdrawn$/ }),
	).toBeVisible();
	await checkAccessibility(page, "withdrawal");
	await page
		.getByRole("button", { name: "Correct source in new cycle" })
		.click();
	await expect(
		page.getByRole("textbox", { name: /Graduate name/ }),
	).toHaveValue("Avery Example");
	...
	await expect(
		page.getByText("Copied source information must be acknowledged again.", {
			exact: false,
		}),
	).toBeVisible();
});

test("administrator reset requires exact typing and returns to an empty role-selection workflow", async ({
	page,
}) => {
	await createExample(page);
	await page.getByRole("button", { name: "Save draft", exact: true }).click();
	...
	await reset.click();
	const dialog = page.getByRole("dialog", {
		name: "Reset the fictional dataset?",
	});
	const confirm = dialog.getByRole("button", {
		name: "Permanently reset fictional dataset",
	});
	await expect(dialog.getByRole("button", { name: "Cancel" })).toBeFocused();
	...
	const phrase = dialog.getByRole("textbox", {
		name: "Type RESET FICTIONAL DEMO to confirm",
	});
	...
	await confirm.click();
	await expect(page.getByRole("status")).toContainText(
		"The fictional dataset was reset",
	);
	await page
		.getByRole("button", { name: /contributor.*Create fictional/ })
		.click();
	await expect(
		page.getByText("No announcements yet.", { exact: false }),
	).toBeVisible();
});
