...
- Application source/configuration/test/lockfile fingerprint: `c4cdbab74ac24d2a6c54eae7d23813edd38618f4ddfcab30deb08ef1a24bfa81`. This is the SHA-256 of a sorted manifest of SHA-256 hashes and relative paths for the 22 non-Markdown application files, excluding dependencies, data, build outputs, and browser reports. It identifies the tested source without treating generated reports as source files.
...
| --- | --- |
| `npm run check` | Passed TypeScript analysis, Biome correctness/hook lint, 37 unit/API/resource tests, and production build. No tests skipped. |
| `npm run test:browser` | 10 of 10 Chromium workflow tests passed. |
| axe scans within browser tests | Zero violations across 14 interface checkpoints. Incomplete automated checks remain distinct from passes. |
| Clean-copy `npm ci --no-fund` followed by `npm run build` | Passed without existing dependencies, build output, or database. npm reported zero known dependency vulnerabilities at installation time; this is not a full security audit. |
...
| Source and deliverable inspection | Runtime browser requests stay on the local origin. Provider input excludes session, consent, and review metadata. User text renders as text. The 34-file deliverable inventory excludes database/build/report/local-agent paths; 71 local Markdown links resolve. A limited credential-pattern scan found no unexpected matches after reviewing the explicit fictional test sentinel. |
