...
IDs are server-generated UUIDs; timestamps are UTC strings set by the server. JSON documents are stored with their schema version and a digest of deterministic JSON serialization (sorted object keys recursively, array order preserved, UTF-8). Hashes identify content; they are not tamper-proof signatures. Enable foreign keys on every database connection. Use schema-version metadata, foreign keys, unique indexes, and transactional application checks for cross-row invariants. The current implementation initializes empty databases transactionally at version 1 and reopens version 1 without reinitializing data or settings. It refuses missing, malformed, or unsupported version metadata in an existing database before modifying it; preserve that file and use a compatible app version or a new `DATABASE_PATH`. No upgrade migration is currently shipped.
...
| `GET /cycles/:cycleId/history` | Owner or reviewer. | `200`; paginated events in chronological order, with revisions, decisions, and safe attempt summaries referenced by that page. Related SQL reads are restricted to those IDs. |
| `GET /cycles/:cycleId/revisions` | Owner or reviewer. | `200`; independently paginated revision metadata in sequence order, without content, plus `next_cursor`. |
| `GET /cycles/:cycleId/revisions/:revisionId` | Owner or reviewer of the containing cycle. | `200`; `{revision}` with immutable content. A revision outside that cycle returns `404`. |
